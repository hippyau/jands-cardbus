...
#define PORT_DELAY  1 // delay before read and after write
#define LCD_DELAY   36 // delay on LCD commands
...
// change bus port pin direction (not the actual CardBus RW signal). 
void dirb(int dir)
{
#ifndef NO_PIN_MAPPING
  for (int c = 0; c < 8; c++)
...
{  
  digitalWrite(DIR, inb & (1 << 0));  // BUF Enable 
  digitalWrite(BUF, inb & (1 << 1));  
  digitalWrite(MUX, inb & (1 << 2));
...
static uint8_t reg_last_addr = 255;
