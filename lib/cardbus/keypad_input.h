...
void keypad_input::edit(lcdModule *_lcd, char *strName, unsigned int *ptrValue, unsigned int _max_value, uint8_t x, uint8_t y)
{
        ...
        if (max_digits <= 3)
        {
                sprintf(display_value, "%03d", (unsigned int)value);
        }
        else if (max_digits == 4)
        {
                sprintf(display_value, "%04d", (unsigned int)value);
        }
...
void keypad_input::updateDisplay()
{
        ...
        {
                if (max_digits <= 3)
                        lcd->printf("%s: %03d ", name, edval);
                else if (max_digits == 4)
                        lcd->printf("%s: %04d ", name, edval);
        }
        else
...
void keypad_input::key(uint8_t pressed)
{
        ...
        else if ((edit_mode) & (pressed == BTN_MINUS))
        {
                edval--;
                if (edval < 0)
                {
